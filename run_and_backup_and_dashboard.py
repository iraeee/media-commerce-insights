...
class EnhancedRunner:
    ...
    def __init__(self):
        ...
        # GitHub 저장소 URL
        self.github_repo = "https://github.com/iraeee/labangba-scraper"
    ...
    def download_db_from_github(self):
        ...
        urls_to_try = [
            f"{base_raw_url}/main/schedule.db",
            f"{base_raw_url}/master/schedule.db"
        ]
        
        for url in urls_to_try:
            try:
                self.print_status(f"다운로드 시도: {url.split('/')[-2]}/{url.split('/')[-1]}", "INFO")
