...
# github_actions/ 등 하위 폴더에서 실행돼도 루트의 labangba_auth / session_cache.json 을 찾도록
...
def fetch_schedule_json(
    ...
    *,
    ...
    # 자동 로그인으로 세션 쿠키 획득 (실패 시 아래 하드코딩 폴백)
    try:
        from labangba_auth import get_cookie
        cookie_str = get_cookie()
    except Exception as _e:
        print(f"자동 로그인 실패, 하드코딩 쿠키 사용: {_e}")
        cookie_str = "_fwb=10E33Fe2dyCoUeh803dAnh.1772173149200"
